...
from handlers import apsched, client, kick_user
            ...
            if huy == 'Отмена':
                ...
                for qut in func:
                    if qut != '' and qut is not None:
                        ...
                        await bot.send_invoice(message.chat.id, title='test', description='test',provider_token='381764678:TEST:49169',currency='rub', need_email=True, prices=PRICE,start_parameter='example',payload='some_invoice')
...
async def successful_payment(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        print(data['input'], 'data.input')
        for check in checker:
            riad = await database_conn.sql_select_command8(check)
            for rut in riad:
                for y in rut:
                    print(y, 'y')
                    if y == sex and message.chat.id == '-1001781599983':
                        print('yes')
